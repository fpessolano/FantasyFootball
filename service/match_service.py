...
class MatchService:
    ...
    def play_multiple_random_matches(self):
        """Play multiple matches between two random teams."""
        print("=" * 80)
        ...
        for match_num in range(1, num_matches + 1):
            if detailed_mode:
                ...
                print(f"Before Match:")
                print(f"  🏠 {team1.name}: Elo {team1.elo_rating:.0f}")
                print(f"  ✈️  {team2.name}: Elo {team2.elo_rating:.0f}")
            else:
                # Fast mode - just show progress
                if match_num == 1 or match_num % 10 == 0 or match_num == num_matches:
                    print(f"\r🎮 Simulating matches... {match_num}/{num_matches}", end="", flush=True)
