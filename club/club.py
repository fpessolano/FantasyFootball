...
import json
import pandas as pd
import formation as fmt

# TODO working fileto add player stats support
...
class Club:
  ...
  def __init__(self,
    ...
    self.team = fmt.Formation()
    try:
      if full_stats:
        self.name = full_stats["name"]
        self.__elo = full_stats["_Team__elo"]
        self.__old_elo = full_stats["_Team__old_elo"]
        self.played = full_stats["played"]
        self.goals = full_stats["goals"]
        self.stats = full_stats["stats"]
        self.stars = full_stats["stars"]
        self.result_streak = full_stats["result_streak"]
        return
    except:
      pass
    self.name = name
  ...
  def elo_from_stars(self, stars, reset):
    ...
    """
    new_elo = Club.__min_elo + 2.05 * stars * Club.__elo_half_step
    if reset:
